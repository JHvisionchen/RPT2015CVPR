...
function [img_files, pos, target_sz, ground_truth, video_path] = load_video_info(base_path, video)
	...
	%see if there's a suffix后缀、下标, specifying one of multiple targets, for
	...
	try
		ground_truth = textscan(f, '%f,%f,%f,%f', 'ReturnOnError',false); %按指定格式读取文件数据 
	catch  % try different format (no commas)
		frewind(f);% 将光标移到文件开始位置
		ground_truth = textscan(f, '%f %f %f %f');  
	end
	ground_truth = cat(2, ground_truth{:});% 连接向量成矩阵
	fclose(f);
	...
	frames = {'David', 300, 770;
			  ...
			  'Freeman4', 1, 283};
	% find()找到列表中非0的元素
	idx = find(strcmpi(video, frames(:,1)));% strcmpi() 大小写不敏感的字符串对比函数
	...
	if isempty(idx),
		...
		if exist(sprintf('%s%04i.png', video_path, frames{idx,2}), 'file'),% exist()判断是否存在；sprintf()按指定格式写入字符串
		...
		img_files = cellstr(img_files);% 根据字符串产生数组
	end
